function createStartEndBlock(x, y, width, height) {
    var h = 20;
    svgAppendix =  `<rect x="${x}" y="${y}" width="${width}" height="${20}" stroke="black" fill="lightblue" />
        <text x="${x + width / 2}" y="${y + height / 2}" font-size="14px" fill="black" text-anchor="middle">Start</text>`;
    svgAppendix +=  `<rect x="${x}" y="${y + height - 20}" width="${width}" height="${20}" stroke="black" fill="lightblue" />
        <text x="${x + width / 2}" y="${y + height / 2}" font-size="14px" fill="black" text-anchor="middle">Start</text>`;
}
