...
function createDecisionBlock(x, y, width, height, question, answerA, answerB) {
    ...
    svgAppendix = `<polygon points="${x},${y} ${x + width},${y} ${x + width / 2},${y + h}" stroke="black" fill="lightyellow" />
        ...
        <text x="${x + 3 * width / 4}" y="${y + h}" font-size="14px" fill="black" text-anchor="middle">${answerB}</text>
        <text x="${x + width / 4}" y="${y + h}" font-size="14px" fill="black" text-anchor="middle">${answerA}</text>`;
