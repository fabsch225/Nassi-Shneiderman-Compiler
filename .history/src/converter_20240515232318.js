function convert(x, y, width, height, tree) {
    ...
    for (let block of tree) {
        if (block.type == 0) { // sequential statement - function call
            ...
            var block = createDecisionBlock(x, current_y, width, h, block.name + '(' + block.arguments[0] + ')', "TRUE", "FALSE");
            svgAppendix += block.svgAppendix;
            var bounds1 = block.children[0];
            convert(bounds1.start_x, bounds1.start_y, bounds1.width, bounds1.height, block.branches[0]);
        }
