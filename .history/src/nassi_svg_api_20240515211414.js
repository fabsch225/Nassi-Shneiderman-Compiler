...
function createSequenceBlock(x, y, width, height) {
    svg = `<rect x="${x}" y="${y}" width="${width}" height="${height}" stroke="black" fill="lightgreen" />
...
function createDecisionBlock(x, y, width, height) {
    svg = `<polygon points="${x},${y + height / 2} ${x + width / 2},${y} ${x + width},${y + height / 2} ${x + width / 2},${y + height}" stroke="black" fill="lightyellow" />
...
function createLoopBlock(x, y, width, height) {
    svg = `<rect x="${x}" y="${y}" width="${width}" height="${height}" stroke="black" fill="lightcoral" />
...
function createJumpBlock(x, y, width, height) {
    svg = `<rect x="${x}" y="${y}" width="${width}" height="${height}" stroke="black" fill="lightgrey" />
...
function createSwitchCaseBlock(x, y, width, height, cases) {
    ...
    let svg = `<rect x="${x}" y="${y}" width="${width}" height="${height}" stroke="black" fill="lightcyan" />`;
    
    cases.forEach((item, index) => {
        const caseY = y + caseHeight * (index + 1);
        svg += `<text x="${x + width / 2}" y="${caseY}" font-size="14px" fill="black" text-anchor="middle">${item}</text>`;
    });
    
    svg += `<text x="${x + width / 2}" y="${y + 20}" font-size="14px" fill="black" text-anchor="middle">Switch</text>`;
